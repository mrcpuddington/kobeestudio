...
class Buzzard():
    ...
    def create_v6_footprint(self, parm_text=None, output_layer=None):
        """Return a KiCad footprint on the requested output layer.

        Back-side artwork is emitted as a B.Cu-owned footprint and mirrored
        once by svg2mod while its primitives remain on the requested B.*
        ...
        mod = Svg2ModExportLatestCustom(
            ...
            center=True,
            params=parm_text,
            bottom_side=output_layer.startswith("B."),
        )
        if output_layer == "F.Cu/F.Mask":
            ...
            mod.add_svg_element(self.svgText, layer=output_layer)
        mod.write()
...
class Svg2ModExportLatestCustom( Svg2ModExportLatest ):
    ...
    """.format(
                ...
                "Generated with Kobee Studio (evolved from KiBuzzard)", #2
                self.params, #3
                footprint_layer, #4
            )
        )
    ...
    def _write_modules(self):
        # Svg2ModExport's ``front`` flag both chooses the owning side and
        # mirrors X coordinates.  Use it exactly once for bottom labels.
        self._write_module(front=not self.bottom_side)
